...
async function runViewport(browser, name, viewport) {
  ...
  const initial = await page.evaluate(() => {
    ...
    const tileReadability = [...surfaces.live.querySelectorAll('.listing-tile')].slice(0, 14).map(tile => {
  ...
  await page.locator('#dripfeed-root [data-action="repack"]').click();
  ...
  assert(latentProof.latentCount > 0, `${name}: repack did not create a real latent member.`);
  ...
  assert(latentProof.latentTransform !== 'none' && latentProof.latentScale < initial.liveTransform.scaleX, `${name}: latent publication is not rendered behind live.`);
...
try {
  ...
  console.log('Dripfeed chamber integration rendered proof passed: initial, reader-open and latent desktop/mobile evidence retained.');
} finally {
  await browser.close();
}
